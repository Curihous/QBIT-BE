...
public class CreateOrderRequest {
    ...
    @Schema(description = "주문 방향 (buy: 매수, sell: 매도)")
    @NotBlank(message = "주문 방향은 필수입니다")
    @Pattern(regexp = "^(buy|sell)$")
    @JsonProperty("side")
    private String side;

    @Schema(description = "주문 유형 (market: 시장가, limit: 지정가, stop: 손절, stop_limit: 손절 지정가)")
    @NotBlank(message = "주문 유형은 필수입니다")
    @Pattern(regexp = "^(market|limit|stop|stop_limit)$")
    @JsonProperty("type")
    private String type;

    @Schema(description = "주문 유효기간 (day: 당일, gtc: 체결될 때까지, ioc: 즉시 체결 또는 취소, fok: 전량 체결 또는 취소)", defaultValue = "day")
    @Pattern(regexp = "^(day|gtc|ioc|fok)$")
    @JsonProperty("time_in_force")
    @Builder.Default
    private String timeInForce = "day";

    @Schema(description = "지정가 (limit 주문 시 필수)")
    @JsonProperty("limit_price")
    private BigDecimal limitPrice;

    @Schema(description = "손절가 (stop 주문 시 필수)")
