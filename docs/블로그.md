> 이 글은 졸업 프로젝트로 진행한 큐빗(QBIT) 프로젝트에서 실시간 암호화폐 시장 데이터 처리 시스템을 구축하며 겪은 경험을 공유한다.
...
이는 몇 년 전 크게 주목받았던 MSA의 개념과도 유사해보인다. 독립적인 서비스를 연결하는 구조인 MSA 그 기반에는 DDD라는 설계 사상이 자리하고 있다. DDD는 Loose Coupling(느슨한 결합)과 High Cohesion(높은 응집)이라는 핵심 원칙을 통해 서비스 간 경계를 명확히 하고, 구조적 분리를 가능하게 한다. 이러한 원칙은 MSA 설계 시 반드시 고려해야 할 중요한 기준이지만, DDD를 적용한다고 해서 MSA의 완성도가 자동으로 보장되는 것은 아니다.
...
# 2. 실시간 암호화폐 시장 데이터 처리 시스템의 구현 및 개선

앞서 큐빗의 아키텍처 설계 철학을 다소 상세히 설명한 것은, 이 글의 메인 주제인 **실시간 암호화폐 시장 데이터 처리 시스템의 구현 및 개선 과정**을 설명하기 위한 기반이었다.

큐빗은 '초보 투자자를 위한 모의투자 결과 AI분석·이론 연계 학습 서비스'로서 모의투자 기능 제공 시 실시간 시장 데이터를 제공하는 것이 핵심이다. 사용자는 주식이나 암호화폐의 실시간 호가창, 체결가, 시세 변동 등을 즉시 확인하며 매매를 결정하므로, **실시간 데이터 처리 시스템의 설계는 서비스의 사용자 경험을 좌우하는 중요한 요소**였다.

## 2-1. 초기 아키텍처: Redis Pub/Sub

큐빗은 **WebSocket 기반 아키텍처**를 채택하여 실시간 시장 데이터를 제공한다. Binance(암호화폐)와 Massive.io(미국주식)의 WebSocket API를 활용하여 호가창, 시세, 체결 데이터를 실시간으로 수신하고, 클라이언트에게 브로드캐스트하는 구조이다.

프로젝트 초기에는 Redis Pub/Sub을 사용하여 실시간 데이터를 처리했는데, 그 이유는 다음과 같다:
...
2. **Consumer (`qbit-api-app/consumer/TradeUpdateConsumer`)**: Streams에서 메시지를 수신하여 도메인 서비스를 호출
...
#### 1. 고부하 처리 능력 검증 (testHighLoadThroughput)
...
```76:106:qbit-realtime-app/src/test/java/com/curihous/qbit/realtime/performance/RedisStreamsPerformanceTest.java
    @Test
...
```132:204:qbit-realtime-app/src/test/java/com/curihous/qbit/realtime/performance/RedisStreamsPerformanceTest.java
...
**성능 평가**: 초당 200건의 메시지 처리 시 평균 93ms, P95 218ms의 응답 시간을 달성하여 실시간 거래 시스템으로서 충분한 성능을 보여주었습니다. 이는 사용자가 체감할 수 있는 실시간성을 제공하며, 모의투자 시스템의 요구사항을 만족합니다.
...
#### 2. 장애 복구 검증 (testFailureRecovery)
...
```211:300:qbit-realtime-app/src/test/java/com/curihous/qbit/realtime/performance/RedisStreamsPerformanceTest.java
    @Test
    @DisplayName("장애 복구 테스트 - 워커 중단 후 재시작 시 데이터 손실 없이 복구")
    void testFailureRecovery() throws InterruptedException {
        log.info("=== 장애 복구 테스트 시작 ===");
        ...
        log.info("초기 메시지 {}개 발행 완료", initialMessages);

        // 2단계: Consumer가 일부 메시지 처리 (ACK 전까지)
        Thread.sleep(500);
        
        // 3단계: Consumer 중단 시뮬레이션 (워커 중단)
        log.info("Consumer 중단 시뮬레이션");
        Thread.sleep(1000);

        // 4단계: 추가 메시지 발행 (워커 중단 중)
        int additionalMessages = 50;
        for (int i = initialMessages + 1; i <= initialMessages + additionalMessages; i++) {
            TradeUpdateEvent event = createTestEvent(i);
            tradeUpdateProducer.publishTradeUpdate(event);
        }
        log.info("워커 중단 중 추가 메시지 {}개 발행 완료", additionalMessages);

        // 5단계: Consumer 재시작 및 메시지 복구 확인
        log.info("Consumer 재시작 및 메시지 복구 시작");
        
        AtomicInteger recoveredCount = new AtomicInteger(0);
        Set<String> processedMessageIds = ConcurrentHashMap.newKeySet();

        // Pending 메시지 확인 (ACK되지 않은 메시지)
        ...
        log.info("Pending 메시지 수: {}", pendingMessages != null ? pendingMessages.getTotalPendingMessages() : 0);
        ...
        if (pendingMessages != null && pendingMessages.getTotalPendingMessages() > 0) {
            ...
            if (messages != null) {
                for (var message : messages) {
                    if (!processedMessageIds.contains(message.getId().getValue())) {
                        processedMessageIds.add(message.getId().getValue());
                        recoveredCount.incrementAndGet();
                        redisTemplate.opsForStream().acknowledge(CONSUMER_GROUP, message.getId());
                    }
                }
            }
        }
...
-   [Redis Streams 소개](https://redis.io/docs/data-types/streams/)
